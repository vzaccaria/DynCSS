...
(function(){
  var builtIn, blue, red, normal, coreDebugMessage, coreHeavyDebugMessage, dynCss;
  builtIn = require('./lib');
  blue = '#3498db';
  red = '#c0392b';
  normal = 'black';
  coreDebugMessage = function(m){
    if (window.dynCss.config.debug) {
      return console.log("%cdyncss-core : %c" + m, "color: " + blue, "color: " + normal);
    }
  };
  coreHeavyDebugMessage = function(m){
    if (window.dynCss.config.debug) {
      return console.log("%cdyncss-core : %c" + m, "color: " + red, "color: " + normal);
    }
  };
            ...
            function fn$(i){
              ...
              for (i$ = 0, len$ = (ref$ = act).length; i$ < len$; ++i$) {
                ...
                if (r = /set-state-(.+)/.exec(a.originalProperty)) {
                  ...
                  if (windowDi.dynCss.config.debug) {
                    coreDebugMessage("Assigning to " + sct + "." + a.property + " value " + a.funct());
                  }
              ...
              for (k in css) {
                ...
                if ((isInitialPhase && isNotHidden) || (isChanged && isNotHidden) || (isChanged && isVisibilityToggled)) {
                  if (windowDi.dynCss.config.debug) {
                    coreHeavyDebugMessage(sct + " - " + k + " transition (" + ((ref$ = this.oldValue) != null ? ref$[k] : void 8) + " --> " + css[k] + ")");
                  }
...
(function(){
  ...
  window.dynCss.api.initVariable = function(vr, value){
    return window.dynCss.lib[vr] = value;
  };
  window.dynCss.api.setVariable = function(vr, value){
    window.dynCss.api.initVariable(vr, value);
    return scrollHandler();
  };
  window.dynCss.api.initToggle = function(vr, value1, value2){
    ...
    return entryDebugMessage("Initialising variable " + vr + " to " + value1);
  };
  window.dynCss.api.toggle = function(vr){
    var vv, v1, v2;
    ...
    if (vv === v1) {
      ...
      return entryDebugMessage("Setting " + vr + " to " + v2);
    } else {
      window.dynCss.api.setVariable(vr, v1);
      return entryDebugMessage("Setting " + vr + " to " + v1);
    }
  };
}).call(this);
