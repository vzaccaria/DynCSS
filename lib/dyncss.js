...
(function(){
  var debug, perspective, sat, asPercentageOf, asRemainingPercentageOf, shouldDisappear, shouldAppear, selectFrom, ifThenElse, isVerticallyVisible, topOf, bottomOf, leftOf, rightOf, _module;
  debug = false;
  ...
  bottomOf = function(el){
    if (el !== window) {
      return jQuery(el).offset().top + parseInt(jQuery(el).css('margin-bottom')) + jQuery(el).innerHeight();
    } else {
