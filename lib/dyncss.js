...
(function(){
  ...
  dynCss = function(windowDi, documentDi, jqDi){
    var innerModule, breakpoint, setNamedBreakpoints, setBreakpoints, camelize, getScrollExpression, transcompileFunction, buildHandlers;
    ...
    windowDi.dynCss.api = {
      ...
      forceRedraw: function(){
        ...
        try {
          return ss.addRule('.xxxxxx', 'position: relative');
        } catch (e$) {
          return e = e$;
        }
      },
      forceRedrawBrute: function(){
        return $(window).hide().show();
      }
    };
    ...
    getScrollExpression = function(d){
      var results, property, expression;
    ...
    transcompileFunction = function(body){
      if (windowDi.dynCss.config.debug) {
        console.log(body);
      }
      ...
      function fn$(next){
        return (function(act, scopedSel){
          return function(changed){
            ...
            function fn$(i){
              var css, i$, ref$, len$, a, r, cc, k, v, isInitialPhase, isChanged, isNotHidden, isVisibilityToggled, newValue;
              ...
              for (k in css) {
                v = css[k];
                isInitialPhase = this.oldValue == null || ((ref$ = this.oldValue) != null ? ref$[k] : void 8) == null;
                isChanged = this.oldValue != null && this.oldValue[k] != null && css[k] !== this.oldValue[k];
                isNotHidden = (fn$());
                isVisibilityToggled = k === 'display';
                if ((isInitialPhase && isNotHidden) || (isChanged && isNotHidden) || (isChanged && isVisibilityToggled)) {
                  if (windowDi.dynCss.config.debug) {
                    console.log(sct + " - " + k + " - " + ((ref$ = this.oldValue) != null ? ref$[k] : void 8) + " - " + css[k]);
                  }
                  windowDi.dynCss.el.css((ref$ = {}, ref$[k + ""] = v, ref$));
                  this.oldValue == null && (this.oldValue = {});
                  this.oldValue[k] = v;
                  changed = true;
                }
              }
              return newValue = JSON.stringify(css);
              function fn$(){
                switch (false) {
                case !!windowDi.dynCss.config.dontComputeInvisible:
                  return true;
                case windowDi.dynCss.el.css('display') === 'none':
                  return true;
                default:
                  return false;
                }
...
(function(){
  var cssParse, builtIn, dynCss, _q, buildHandlers, blue, red, normal, entryDebugMessage, scrollHandler, refreshHandler, decimate, iOS, counter, lt, fixedTtc, installCustomRaf, installScrollHandler, parseCss, _loaded_d, _loaded_p, results, pArray, r;
  ...
  red = '#c0392b';
  normal = 'black';
  entryDebugMessage = function(m){
    if (window.dynCss.config.debug) {
      return console.log("%cdyncss-entry: %c" + m, "color: " + blue, "color: " + normal);
    }
  };
  refreshHandler = function(changed){
    if (changed) {
      return window.dynCss.api.forceRedraw();
    }
  };
  decimate = 1;
  ...
  installCustomRaf = function(){
    return window.customRequestAnimationFrame = function(cb){
      var ct, ttc;
      ct = new Date().getTime();
  ...
  installScrollHandler = function(options){
    ...
    if ((options != null ? options.onlyOnResize : void 8) != null) {
      return window.onresize = scrollHandler;
    } else {
      if ((options != null ? options.onlyOnStart : void 8) == null) {
  ...
  entryDebugMessage("Scanning for css");
  parseCss = function(n){
    var _d;
    entryDebugMessage("Loading " + n.href);
    _d = _q.defer();
    if (n.href != null) {
      $.get(n.href, function(it){
        var rules;
        entryDebugMessage("Loaded " + n.href);
  ...
  window.onload = function(){
    entryDebugMessage("Content loaded");
    return _loaded_d.resolve();
  };
  $(document).ready = function(){
    return entryDebugMessage("Document parsed.");
  };
  ...
  pArray = (function(){
    ...
    for (i$ = 0, len$ = (ref$ = results).length; i$ < len$; ++i$) {
      r = ref$[i$];
      results$.push(parseCss(r));
    }
    return results$;
  }()).concat([_loaded_p]);
  _q.all(pArray).then(function(){
    entryDebugMessage("Initializing handler");
    return scrollHandler();
  });
}).call(this);

},{"./core":1,"./lib":3,"css-parse":4,"q":6}],3:[function(require,module,exports){
(function(){
  var debug, perspective, sat, asPercentageOf, asRemainingPercentageOf, shouldDisappear, transitionToOne, shouldAppear, selectFrom, ifThenElse, isVerticallyVisible, topOf, bottomOf, leftOf, rightOf, _module;
  debug = false;
  ...
  asPercentageOf = curry$(function(x$, y$){
    return x$ / y$;
  });
  ...
  shouldDisappear = function(context){
    ...
    isHigherThan = context.isHigherThan, isLowerThan = context.isLowerThan, wn = context.when;
  ...
  transitionToOne = function(context, power){
    var int, vv, direction, start, stop, val, orig, pp;
    power == null && (power = 1);
    start = context.start, stop = context.stop, val = context.when;
    orig = val;
    pp = (function(){
      switch (false) {
      case !(start < stop && val < start):
        return 0;
      case !(start < stop && val > stop):
        return 1;
      case !(start < stop):
      ...
      case !(start > stop && val < stop):
        return 0;
      case !(start > stop):
        return 1 - (val - stop) / (start - stop);
      }
    }());
    vv = sat(pp);
    return Math.pow(vv, power);
  };
  ...
  isVerticallyVisible = function(el, threshold){
    ...
    threshold == null && (threshold = w.height() / 3);
    value = (function(){
      switch (false) {
  ...
  topOf = function(el){
    if (el !== window) {
      return jQuery(el).offset().top - $(window).scrollTop();
    } else {
      return 0;
    }
  };
  bottomOf = function(el){
    if (el !== window) {
      return jQuery(el).offset().top - $(window).scrollTop() + parseInt(jQuery(el).css('margin-bottom')) + jQuery(el).innerHeight();
    } else {
  ...
  leftOf = function(el){
    if (el !== window) {
      return jQuery(el).offset().left + parseInt(jQuery(el).css('margin-right'));
    } else {
      return 0;
    }
  };
  rightOf = function(el){
    if (el !== window) {
      return jQuery(el).offset().left + parseInt(jQuery(el).css('margin-right')) + jQuery(el).innerWidth();
    } else {
      return $(window).width();
    }
  };
  _module = function(){
    ...
    iface = {
      ...
      'if': ifThenElse,
      fixedTopEdge: topOf,
      fixedBottomEdge: bottomOf,
      fixedRightEdge: rightOf,
      fixedLeftEdge: leftOf,
      pos: function(el){
        return $(el).offset();
      },
      fixedHorizontalCenter: function(it){
        return (rightOf(it) + leftOf(it)) / 2;
      },
      fixedVerticalCenter: function(it){
        return (topOf(it) + bottomOf(it)) / 2;
      },
      morph: function(c, v1, v2){
        var vv;
        vv = v1 * (1 - c) + v2 * c;
        return vv;
      },
...
}).call(this,require("/Users/zaccaria/development/github/DynCSS/node_modules/gulp-browserify/node_modules/browserify/node_modules/insert-module-globals/node_modules/process/browser.js"))
},{"/Users/zaccaria/development/github/DynCSS/node_modules/gulp-browserify/node_modules/browserify/node_modules/insert-module-globals/node_modules/process/browser.js":5}]},{},[2])